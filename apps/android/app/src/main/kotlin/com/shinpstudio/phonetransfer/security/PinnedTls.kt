...
class PinnedTrustManager(private val pin: String) : X509TrustManager {
    init { require(pin.matches(Regex("[a-f0-9]{64}"))) }

    override fun checkServerTrusted(chain: Array<X509Certificate>, authType: String) {
        val leaf = chain.firstOrNull() ?: throw CertificateException("MISSING_CERTIFICATE")
        leaf.checkValidity()
        if (leaf.basicConstraints != -1 || leaf.keyUsage?.getOrNull(0) != true ||
    ...
    fun client(endpoint: String, pin: String, identity: ClientIdentity? = null): OkHttpClient {
        ...
        return OkHttpClient.Builder()
            ...
            .hostnameVerifier { host, session ->
                host == target.host && runCatching {
                    trust.checkServerTrusted(session.peerCertificates.map { it as X509Certificate }.toTypedArray(), "EC")
                }.isSuccess
            }
            ...
            .addInterceptor { chain ->
                ...
                check(url.isHttps && url.host == target.host && url.port == target.port) { "ENDPOINT_MISMATCH" }
...
private class ClientKeyManager(private val identity: ClientIdentity) : X509ExtendedKeyManager() {
    override fun getClientAliases(keyType: String?, issuers: Array<out Principal>?): Array<String>? =
        if (keyType == "EC") arrayOf("client") else null
    override fun chooseClientAlias(keyType: Array<out String>?, issuers: Array<out Principal>?, socket: Socket?): String? =
        if (keyType?.contains("EC") == true) "client" else null
    override fun getCertificateChain(alias: String?): Array<X509Certificate>? =
        if (alias == "client") arrayOf(identity.certificate) else null
    override fun getPrivateKey(alias: String?): PrivateKey? = if (alias == "client") identity.key else null
