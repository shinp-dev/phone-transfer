...
class FileTransferService : Service() {
    ...
    override fun onCreate() {
        ...
        manager.createNotificationChannel(
            NotificationChannel(CHANNEL_ID, "ファイル転送", NotificationManager.IMPORTANCE_LOW).apply {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        if (intent?.action == ACTION_CANCEL) {
            ...
            if (transferJob?.isActive != true) stopSelf(startId)
        ...
        val currentOperation = intent.getStringExtra(EXTRA_OPERATION_ID) ?: UUID.randomUUID().toString()
        val kind = if (intent.action == ACTION_UPLOAD) TransferKind.Upload else TransferKind.Download
    ...
    private suspend fun runTransfer(intent: Intent, currentOperation: String, kind: TransferKind) {
        ...
        val pc = SavedPcStore.get(this).read().firstOrNull { it.deviceId == deviceId }
            ?: throw FileTransferException("PC_NOT_FOUND", false, "The paired PC is no longer saved.")
        ...
        if (kind == TransferKind.Upload) {
            ...
            try {
                val result = repository.upload(pc, shareId, directory, uri) { done, total ->
                    publishProgress(currentOperation, kind, done, total)
                }
                TransferStatusBus.complete(currentOperation, kind, result.fileName)
            } finally {
                if (persisted) releasePersistable(uri, Intent.FLAG_GRANT_READ_URI_PERMISSION)
            }
        } else {
            ...
            try {
                val result = repository.download(pc, shareId, remotePath, uri) { done, total ->
                    publishProgress(currentOperation, kind, done, total)
                }
                TransferStatusBus.complete(currentOperation, kind, result.fileName)
            } finally {
                if (persisted) releasePersistable(uri, Intent.FLAG_GRANT_WRITE_URI_PERMISSION)
            }
        }
    }
    ...
    private fun publishProgress(operationId: String, kind: TransferKind, done: Long, total: Long) {
        ...
        if (done == total || now - lastNotificationAt >= NOTIFICATION_INTERVAL_MS) {
            ...
            getSystemService(NotificationManager::class.java).notify(
                NOTIFICATION_ID,
                notification(kind, done, total, if (kind == TransferKind.Upload) "PCへ送信中" else "PCから受信中")
            )
        }
    }

    private fun notification(kind: TransferKind, done: Long, total: Long, text: String): Notification {
        ...
        val progress = if (determinate) ((done.coerceIn(0, total) * 1000) / total).toInt() else 0
        val title = if (kind == TransferKind.Upload) "Phone Transfer - 送信" else "Phone Transfer - 受信"
        return Notification.Builder(this, CHANNEL_ID)
            ...
            .addAction(Notification.Action.Builder(R.drawable.ic_transfer, "中止", cancel).build())
            .build()
    }
    ...
    private fun Intent.requireString(name: String): String =
        getStringExtra(name)?.takeIf { it.isNotEmpty() }
            ?: throw FileTransferException("INVALID_TRANSFER_INTENT", false, "The transfer request is incomplete.")
    ...
    companion object {
        ...
        fun startUpload(context: Context, deviceId: String, shareId: String, directory: String, source: Uri) {
        ...
        fun startDownload(context: Context, deviceId: String, shareId: String, remotePath: String, destination: Uri) {
            val intent = Intent(context, FileTransferService::class.java)
        ...
        fun cancel(context: Context) {
            context.startService(Intent(context, FileTransferService::class.java).setAction(ACTION_CANCEL))
        }
    }
}
