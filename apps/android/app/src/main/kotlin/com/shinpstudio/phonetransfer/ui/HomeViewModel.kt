...
class HomeViewModel(application: Application) : AndroidViewModel(application) {
    ...
    init {
        runOperation { mutableState.update { it.copy(pcs = repository.saved()) } }
        viewModelScope.launch {
            TransferStatusBus.state.collect { transfer ->
                mutableState.update { current ->
                    val label = when (transfer) {
                        ...
                        is TransferServiceState.Failed -> "ファイル転送に失敗しました (${transfer.code})"
                    ...
                    try {
                        ...
                        mutableState.update { current ->
                            current.copy(
                                ...
                                connectionLabel = if (current.connectionLabel == "PC未接続") {
    ...
    fun openDirectory(entry: FileEntry) = runOperation {
        ...
        mutableState.update {
            it.copy(entries = entries, currentPath = entry.relativePath, connectionLabel = "${pc.displayName} を参照中")
        }
    }
    ...
    fun goUp() = runOperation {
        ...
        mutableState.update { it.copy(entries = entries, currentPath = parent) }
    }
    ...
    fun upload(uri: Uri) {
        ...
        try {
            FileTransferService.startUpload(getApplication(), deviceId, share.id, current.currentPath, uri)
            mutableState.update { it.copy(connectionLabel = "PCへの送信を開始しました") }
        } catch (_: RuntimeException) {
            mutableState.update { it.copy(connectionLabel = "ファイル転送サービスを開始できませんでした") }
        }
    }
    ...
    fun download(entry: FileEntry, destination: Uri) {
        ...
        try {
            RemotePathRules.validate(entry.relativePath)
            FileTransferService.startDownload(getApplication(), deviceId, share.id, entry.relativePath, destination)
            mutableState.update { it.copy(connectionLabel = "PCからの受信を開始しました") }
        } catch (_: RuntimeException) {
            mutableState.update { it.copy(connectionLabel = "ファイル転送サービスを開始できませんでした") }
        }
    }
    ...
    fun cancel() {
        if (state.value.transfer is TransferServiceState.Running) {
            ...
            mutableState.update { it.copy(connectionLabel = "ファイル転送を中止しています…") }
            return
        }
        operation?.cancel()
        mutableState.update {
        ...
        if (share == null) {
            mutableState.update {
                it.copy(
                    ...
                    entries = emptyList(),
                    currentPath = "",
                    connectionLabel = "${pc.displayName} に接続しました（PC側の受信フォルダは未設定です）"
                )
            }
            return
        }
    ...
    private fun runOperation(block: suspend () -> Unit) {
        if (operation?.isCompleted == false || state.value.transfer is TransferServiceState.Running) return
        operation = viewModelScope.launch {
            ...
            try {
                ...
                mutableState.update {
                    it.copy(connectionLabel = "登録の有効期限が切れました。PCで新しいQRを表示してください。")
                }
            } catch (error: CancellationException) {
                throw error
            } catch (error: FileTransferException) {
                mutableState.update { it.copy(connectionLabel = "PCのファイル操作に失敗しました (${error.code})") }
            } catch (_: Exception) {
                mutableState.update {
                    it.copy(
                ...
                mutableState.update { it.copy(busy = false, comparisonCode = null) }
            }
        }
    }
}
