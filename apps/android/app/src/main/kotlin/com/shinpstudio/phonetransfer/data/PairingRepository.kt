...
@Serializable
data class SavedPc(val deviceId: String, val displayName: String, val endpoint: String, val pin: String)
...
class PairingRepository(private val context: Context) {
    ...
    private fun readSaved(): List<SavedPc> {
        ...
        return json.decodeFromString<List<SavedPc>>(file.openRead().use { it.readBytes().toString(Charsets.UTF_8) }).also { pcs ->
    ...
    suspend fun forget(id: String) = withContext(Dispatchers.IO) { writeSaved(readSaved().filterNot { it.deviceId == id }) }
    ...
    suspend fun connect(pc: SavedPc): ServerInfo = withContext(Dispatchers.IO) {
        ...
        try {
            val info = json.decodeFromString<ServerInfo>(execute(client, Request.Builder().url("${pc.endpoint}/api/v1/info").build(), 200))
            check(info.deviceId == pc.deviceId && info.protocolVersion == 1L) { "SERVER_IDENTITY_MISMATCH" }
            info
        } finally { close(client) }
    }

    suspend fun pair(payload: String, onCode: suspend (String) -> Unit): SavedPc = withContext(Dispatchers.IO) {
        ...
        val identity = ClientIdentity.load(context)
        val transcript = PairingProof.transcript(identity.deviceId, "Android", invitation.token, identity.certificate.encoded)
        val request = PairingRequest(identity.deviceId, "Android", invitation.token,
            Base64.getEncoder().encodeToString(identity.certificate.encoded), PairingProof.sign(identity.key, transcript))
        ...
        try {
            withTimeout(120_000) {
                ...
                val proof = PairingProof.sign(identity.key, "phone-transfer/pairing-status/v1\n$requestId")
                while (status.status == "pending") {
                    delay(1500)
                    status = json.decodeFromString<PairingStatus>(execute(client,
                        Request.Builder().url("$route/$requestId").header("X-Pairing-Proof", proof).build(), 200))
                    check(status.requestId == requestId)
                }
                check(status.status == "approved") { "PAIRING_DENIED_OR_EXPIRED" }
                val pc = SavedPc(invitation.deviceId, invitation.displayName, invitation.apiEndpoint.toString(), invitation.serverSpkiSha256)
                connect(pc)
                ensureActive()
                writeSaved(pcs + pc)
                pc
    ...
    private suspend fun execute(client: OkHttpClient, request: Request, expected: Int): String = suspendCancellableCoroutine { continuation ->
        val call = client.newCall(request)
        continuation.invokeOnCancellation { call.cancel() }
        call.enqueue(object : Callback {
            override fun onFailure(call: Call, error: IOException) {
                if (continuation.isActive) continuation.resumeWithException(IOException("CONNECTION_FAILED"))
            }
