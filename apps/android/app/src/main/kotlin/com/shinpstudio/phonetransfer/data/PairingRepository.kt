...
class PairingRepository(context: Context) {
    ...
    suspend fun forget(id: String) = withContext(Dispatchers.IO) { savedPcs.remove(id) }

    suspend fun connect(pc: SavedPc): ServerInfo =
    ...
    suspend fun acceptDiscovery(discovered: DiscoveredPc): SavedPc? =
        withContext(Dispatchers.IO) {
            val current =
            ...
            ensureActive()
            savedPcs
                .updateEndpoint(current.deviceId, discovered.endpoint)
