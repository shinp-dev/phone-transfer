...
**現在はペアリング接続の実装段階です。転送可能なMVPではありません。** WindowsトレイからQR表示・端末承認・登録解除、AndroidからQR読取・確認番号表示・mTLS接続確認・PC登録保存/削除が可能です。実機接続の受入試験とmDNS、ファイル・テキスト転送は未完了です。[実装状況](docs/implementation-status.md)で利用可能範囲を確認してください。開発の再開手順は[引き継ぎ資料](docs/handoff.md)にまとめています。
