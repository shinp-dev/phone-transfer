...
2. Every subsequent traversal uses the same single-component relative open, `OBJ_DONT_REPARSE`, `FILE_OPEN_REPARSE_POINT`, and inspection of the returned handle. `FileAttributeTagInfo` rejects every reparse tag, including junctions, volume mount points and file/directory symlinks. No check-then-open by absolute pathname exists.
3. The volume root, configured root ancestry, and operation ancestry remain open without delete sharing. Existing conflicting handles cause failure. Once an open succeeds, ordinary rename, deletion and replacement cannot acquire the necessary incompatible access. Directory write sharing permits the kernel's completion rename; in-place reparse mutation is treated as untrusted, with parent inspection and OBJ_DONT_REPARSE on relative opens. Operations use the already-bound directory object, never resolve its old absolute pathname again. File handles additionally deny write sharing. No `FILE_SHARE_DELETE` or POSIX rename semantics are enabled. Local administrator/kernel compromise and same-user malware remain outside the threat boundary.
...
## Durable extension (PR #11)

The process-owned staging behavior above remains the ordinary `CreateStaging` contract. Durable uploads use a separate `IDurableShareSession` capability: random directory/file tokens, volume + full file ID for every recovery anchor, inspected protected current-user ACL, explicit handle-bound truncate/flush and explicit deletion. Closing durable handles does not abandon bytes. No client-provided staging token or physical staging path is accepted.

See [durable recovery](../architecture/durable-transfer-recovery.md) for database authority, startup proof, terminal cleanup, bounded orphan quarantine, configuration generation and rename/DB crash ordering. This extension retains every existing relative-open, no-reparse, single-link, same-volume and no-replace invariant.
